...
CONF_FLAGS=(
  ...
  ${MEM64:+ -sMAXIMUM_MEMORY=8GB -sALLOW_MEMORY_GROWTH} # memory64: grow linear memory past the 4GB wasm32 ceiling (also covers the MT path, whose branch omits growth)
  -sEXPORT_NAME="$EXPORT_NAME"             # required in browser env, so that user can access this module from window object
  -sEXPORTED_FUNCTIONS=$(node src/bind/ffmpeg/export.js) # exported functions
  -sEXPORTED_RUNTIME_METHODS=$(node src/bind/ffmpeg/export-runtime.js) # exported built-in functions
  -lworkerfs.js
  ${MEM64:+ --pre-js src/bind/ffmpeg/mem64.js} # memory64: switch bind.js into 64-bit pointer mode (must precede bind.js)
